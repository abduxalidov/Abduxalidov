...
export const Chatbot = () => {
  ...
  const processResponse = (userText: string) => {
    ...
    if (text.includes("salom") || text.includes("assalom")) {
      ...
      response = "Narxlar:\nWeb sayt, Branding, Landing page: $20 dan.\nMobil ilova: $150 dan.\nSMM narxlari SMM bo'limida ko'rsatilgan.";
      ...
      response = "SMM xizmatlariga Telegram va Instagram uchun obunachi, ko'rishlar, layklar va Telegram Premium hamda Stars qo'shish kiradi.";
    }

    setTimeout(() => { setMessages(prev => [...prev, { role: 'ai', text: response }]); }, 600);
  };
  ...
  return (
    <>
      <motion.button
        className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-blue-600 to-emerald-500 rounded-full flex items-center justify-center text-white shadow-lg shadow-blue-500/20 z-50"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-6 w-[calc(100vw-32px)] sm:w-[380px] h-[500px] bg-[hsl(var(--background))] border border-white/10 rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden"
          >
            <div className="bg-white/5 border-b border-white/10 p-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-foreground">AI Yordamchi</h3>
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl whitespace-pre-line ${
                    msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-white/10 text-muted-foreground rounded-tl-sm'
                  }`}>
                    {msg.text}
                  </div>
                </div>
            ...
            <form onSubmit={handleSend} className="p-3 border-t border-white/10 bg-black/20 flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 focus:outline-none focus:border-blue-500 text-foreground placeholder:text-muted-foreground"
              />
              <button type="submit" disabled={!input.trim()} className="bg-blue-600 text-white p-2 rounded-xl hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center justify-center w-12">
